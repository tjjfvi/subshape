...
export function map<KI, KO extends KI, VI, VO>(
  $key: Codec<KI, KO>,
  $value: Codec<VI, VO>,
): Codec<ReadonlyMap<KI, VI>, ScaleMap<KO, VO>> {
...
export function set<I, O extends I>($value: Codec<I, O>): Codec<ReadonlySet<I>, ScaleSet<O>> {
  return withMetadata(
...
export class ScaleMap<K, V> implements Map<K, V> {
  ...
  constructor(readonly $key: Codec<K>, entries?: Iterable<[K, V]>) {
    if (entries) {
...
export class ScaleSet<T> implements Set<T> {
  ...
  constructor(readonly $value: Codec<T, unknown>, values?: Iterable<T>) {
    if (values) {
...
function transformKey<K>($key: Codec<K, unknown>, hexMemo: WeakMap<K & object, string>, key: K): Primitive {
