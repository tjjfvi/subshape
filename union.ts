...
export type UnionT<Members extends Transcoder[] = Transcoder[]> = Native<Members[number]>;

/** Decode a union */
export class UnionDecoder<MemberDecoders extends RecordDecoder[]> extends Decoder<UnionT<MemberDecoders>> {
  /**
  ...
  constructor(...memberDecoders: MemberDecoders) {
    super((state) => {
      ...
      return memberDecoders[discriminant]!._d(state) as UnionT<MemberDecoders>;
    });
  }
}

/** Encode a union */
export class UnionEncoder<MemberEncoders extends Encoder[]> extends Encoder<UnionT<MemberEncoders>> {
  /**
   * @param discriminate A function which accepts the decoded value and returns the encoder with which to encode the value
   */
  constructor(
    readonly discriminate: (value: UnionT<MemberEncoders>) => number,
